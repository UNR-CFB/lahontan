...
class Experiment:
    ...
    class Sample:
        ...
        def __init__(self,sampleNumber,inputFile):#,experiment):
        ...
        def getReadNames(self):
            ...
            return files
        def runFastqc(self):
            fastqcFolder = '{}/fastqc.{}'.format(self.samplePath, self.sampleName)
            if not os.path.exists(fastqcFolder):
                os.makedirs(fastqcFolder)
            command1=['fastqc','-t',str(self.Procs),'-o',fastqcFolder,
                    '*.{}.gz'.format(self.Fastq)]
            command2=['cd',fastqcFolder,'&&','unzip','\*.zip']
        ...
        def getPhred(self):
            ...
            phred = 33
            if illumVN >= 1.3 or illumVN < 1.8:
                phred = 64
            else:
                phred = 33
            return phred
        def findOverrepSeq(self):
            fastqcFolder = '{}/fastqc.{}'.format(self.samplePath, self.sampleName)
            ...
            for directory in availableDirs:
                ...
                if len(G) != 0:
                    ...
                    thing = 'no'
                print('There are {} overrepresented sequence in {}'.format(thing,
                                str('{}/{}'.format(fastqcFolder, directory))))
        def runTrimmomatic(self):
            os.chdir(self.samplePath)
            Phred = self.getPhred()
            Reads = self.getReadNames()
            command = r'java -jar $RNASEQDIR/Trimmomatic/trimmomatic-0.35.jar PE -threads {} -phred{} {} {} read1.P.trim.{}.gz read1.U.trim.{}.gz read2.P.trim.{}.gz read2.U.trim.{}.gz ILLUMINACLIP:$RNASEQDIR/Trimmomatic/adapters/TruSeq3-PE-2.fa:2:30:10 LEADING:5 TRAILING:5 SLIDINGWINDOW:4:5 MINLEN:25'
            #procs
            #phred
            #Read1
            #Read2
    ...
    def dostuff(self):
        S = Experiment.Sample(6,self.inputPath)
        S.runTrimmomatic()
